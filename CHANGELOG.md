# [1.2.0](https://github.com/popstas/windows-mqtt/compare/v1.1.1...v1.2.0) (2026-08-18)

**Проект переведён на ES-модули.** Снаружи не изменилось ничего — ни одной новой
кнопки, ни одного нового топика. Изменилось внутри: `"type": "module"` в
`package.json`, и ни одного `require()` в `src/`, `scripts/` и `test/`.

**Highlights**

- `package.json` добавлен в `bundle.resources`. Без него флаг не доезжал бы до
  установленного приложения: Node ищет ближайший `package.json` вверх от
  `_up_/src/index.js`, и на Node ниже 22.7 прочитал бы `src/*.js` как CommonJS.
  Охраняется тестом — иначе сбой был бы молчаливым, `initModules()` глотает
  исключения модулей в лог.
- **OBS чуть не сменил протокол на пустом месте.** Механический перевод
  `require('obs-websocket-js')` в `import` переключил бы интеграцию с JSON на
  MessagePack: в `exports`-карте пакета есть ключ `require`, но нет `import`,
  поэтому голый импорт уходит в `default` — msgpack-сборку. Починено явным
  подпутём `obs-websocket-js/json`. Это единственное место, где перевод изменил
  поведение, и найти его удалось только чтением: тестов на этом пути нет.
- **Два сторожа миграция сделала вакуумными, они починены.** Тест, искавший
  относительные `require()` в `src/`, после перехода не находил ничего и потому
  проходил всегда — от здорового такой тест неотличим. Переписан на импорты и
  проверен подсадкой нарушителя; заодно добавлено утверждение о непустоте, так
  что сломанную регулярку теперь заметит сам тест.
- Тесты больше не подменяют модули через `require.cache`. Конфиг получил точку
  инъекции (`reload()`/`setConfig()`), а robotjs в `keys` приходит четвёртым
  аргументом — благодаря чему три теста нажатий, раньше скипавшихся везде кроме
  Windows, теперь исполняются на любой машине.
- Нативные аддоны и реализации модулей по-прежнему не грузятся при загрузке
  модуля: одна битая нативная зависимость не должна убивать сервер.

**Для тех, кто правит код:** относительные импорты обязаны нести расширение
`.js`; CJS-зависимости импортируются дефолтом и деструктурируются; при переводе
зависимости стоит сверять `import.meta.resolve` с `require.resolve` — расхождение
и есть ловушка, на которой попался OBS. Планка рантайма поднялась до Node 20.11
(`import.meta.dirname`), тесты требуют 22.15. Подробности в `AGENTS.md`.



### Bug Fixes

* **obs:** вернуть OBS на JSON-протокол после ESM-перевода импорта ([c4278ef](https://github.com/popstas/windows-mqtt/commit/c4278efa6471c84dc6e4129aa4fd77c1cbd59c39))

## [1.1.1](https://github.com/popstas/windows-mqtt/compare/v1.1.0...v1.1.1) (2026-08-17)


**Первый пункт меню трея теперь отвечает на вопрос «то ли собралось»:
`v1.1.1 · 01:10`.** Выкатка заменяет бинарь на месте, а версия у всех сборок
между релизами одна — после перезапуска нечем было проверить, что поднялось
именно новое. Сегодняшняя сборка показывает время, несегодняшняя — ещё и дату
(`v1.1.1 · 2026-08-15 23:05`): без даты вчерашнее `23:05` читалось бы как
сегодняшнее, и неудавшаяся выкатка выглядела бы удавшейся.

**Highlights**

- Штамп вшивает `src-tauri/build.rs`. Входы сборки, лежащие вне крейта
  (`../src` с node-частью и `../frontend`), объявлены в `rerun-if-changed`
  руками: без этих строк код пересобирается, а сборочный скрипт нет, и меню
  показывает время прошлой выкатки — врёт ровно там, куда пришли за правдой.
- Сборка с `WINDOWS_MQTT_RELEASE=1` показывает одну версию, без штампа.
  Признак релиза — переменная окружения, а не `debug_assertions`: выкатка
  собирает `--release`, и на профиле штамп пропал бы там, где он и нужен.
- Разметка пункта — чистая функция `version_item_label()`, на неё три теста
  (сегодняшняя сборка, несегодняшняя, релизная): меню в тестах не собрать.
- Префикс `windows-mqtt ` из этого пункта убран — подпись стала одинаковой с
  треями соседних приложений. Заголовки окон (`windows-mqtt vX.Y.Z`) прежние.

Сюда же вошло накопившееся после v1.1.0: окно About, версия в заголовке окна и
лицензия MIT (`2d4000f`), а также профиль `release` без fat-LTO (`44adc34`) —
пересборка после правки одной строки на целевой машине заняла 3,8 с вместо 204.
