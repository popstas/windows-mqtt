...
`src/config.js` меняет форму экспорта на три именованных:
`{ config, reload, setConfig }`.

- `config` — живой объект конфига, идентичность которого сохраняется на всё
  время процесса;
- `reload()` — перечитывает YAML и возвращает **новый** объект, общий `config`
  не трогает;
- `setConfig(obj)` — заменяет содержимое общего `config` **на месте**, точка
  инъекции для тестов (см. §5).

Голый объект конфига перестаёт быть экспортом целиком, а не обрастает
довесками: `reload`/`setConfig`, повешенные на сам объект, попали бы под
`Object.keys(config)` внутри `setConfig` и были бы им же удалены. Потребителей
всего три — `src/helpers.js:1`, `src/server.js:3`, `src/modules/midi.js`, — и
все переходят на `import { config } from './config.js'`.

`setConfig` мутирует на месте по необходимости: `src/helpers.js:1` захватывает
ссылку на объект при загрузке модуля, и подмена ссылки до него бы не доехала.
`reload()`, наоборот, возвращает свежий объект намеренно: сегодня `midi.js`
через cache-busting получал новый объект только себе, а `helpers.js` продолжал
держать свой. Мутация общего объекта из `midi.js` была бы расширением поведения
на весь процесс — за рамками миграции.
...
- Тест «импорт реестра не грузит реализации модулей» переезжает на
  ...
  остаются дословно теми же. Реестр импортируется с суффиксом `?lazy-probe` —
  хук видит только новые загрузки, а без строки запроса модуль уже лежит в
  кэше. Прототип проверен на 22.23: работает без флагов и без
  ExperimentalWarning.
  Это единственный `require.cache`, который переживает подготовительные
  коммиты: `registerHooks` наблюдает только ESM-загрузки, поэтому написать
  замену до перехода на `"type": "module"` невозможно — она переезжает вместе
  с флаг-днём.
...
Один PR, пять коммитов. `"type": "module"` — флаг-день: между ним и переводом
последнего файла зелёного состояния не существует, поэтому дробить его нельзя.
Подготовительные коммиты существуют именно для того, чтобы флаг-день не тащил
на себе ещё и смену тестовых механизмов.

1. **Подготовка, ещё на CJS, тесты зелёные** — три независимо ревьюируемых
   коммита:
   1. `reload()`/`setConfig()` в `src/config.js` (§4) и трое потребителей;
      `power-module-gate` переходит на `setConfig(reload())`.
   2. `deps.robot` в `src/modules/keys.js` (§5); `keys-press-throttle`
      перестаёт скипаться вне Windows.
   3. async `load()` в `src/modules/index.js` (§2) с `await` в
      `src/helpers.js:190`; `modules-registry` и `native-modules`
      подстраиваются.
